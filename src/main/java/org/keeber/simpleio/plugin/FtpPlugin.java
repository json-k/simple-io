	...
	public class FtpSIOFile extends File {
		...
		private ArrayList<File> IOList(Filter filter, String root, int depth) throws IOException {
			...
			for (FTPFile f : client.listFiles(root)) {
				...
				if (f.isDirectory() && filter.isFollowed(ff, depth)) {
		...
		@SuppressWarnings("unchecked") @Override public <T> T open(Class<T> streamType) throws IOException {
			...
			if (streamType == File.READ) {
				...
				return (T) client.retrieveFileStream(path);
			}
			if (streamType == File.WRITE) {
				return (T) new CloseNotifyOutputStream(client.storeFileStream(path));
			}
			return null;
		}
	...
	public class RumpusFileEntryParserFactory implements FTPFileEntryParserFactory {
		...
		public class Parser implements FTPFileEntryParser {
			...
			public FTPFile parseFTPEntry(String listEntry) {
				...
				// ///DATE MODIFIED
				c.set((parsed[6].matches(".*:.*")) ? Calendar.getInstance().get(Calendar.YEAR) : Integer.parseInt(parsed[6]),
						mMap.get(parsed[4].toLowerCase()), Integer.parseInt(parsed[5]),
						(parsed[6].matches(".*:.*")) ? Integer.parseInt(parsed[6].split(":")[0]) : 0,
						(parsed[6].matches(".*:.*")) ? Integer.parseInt(parsed[6].split(":")[1]) : 0);
